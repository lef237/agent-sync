...
読めなかったディレクトリは、意図的に除外したものとは区別します。警告を出してそこだけスキップし、同期自体は続行しますが、`--check` は失敗します。見ていない部分木について「同期済み」とは言えないためです。
...
Skill の同期は symlink を作るため、Windows では開発者モードか管理者権限のシェルが必要です。指示ファイルの同期はどちらでも動きます。
